---
description: Список механик нашего сервера
icon: gear
---

# Механики

***

{% columns %}
{% column %}
{% content-ref url="kalyan.md" %}
[kalyan.md](kalyan.md)
{% endcontent-ref %}
{% endcolumn %}

{% column %}
{% content-ref url="afk.md" %}
[afk.md](afk.md)
{% endcontent-ref %}
{% endcolumn %}
{% endcolumns %}

{% columns %}
{% column %}
{% content-ref url="bank.md" %}
[bank.md](bank.md)
{% endcontent-ref %}
{% endcolumn %}

{% column %}
{% content-ref url="sistema-shtrafov.md" %}
[sistema-shtrafov.md](sistema-shtrafov.md)
{% endcontent-ref %}
{% endcolumn %}
{% endcolumns %}

{% columns %}
{% column %}
{% content-ref url="dubinka-i-veryovka.md" %}
[dubinka-i-veryovka.md](dubinka-i-veryovka.md)
{% endcontent-ref %}
{% endcolumn %}

{% column %}
{% content-ref url="demorgan.md" %}
[demorgan.md](demorgan.md)
{% endcontent-ref %}
{% endcolumn %}
{% endcolumns %}

{% columns %}
{% column %}
{% content-ref url="bloki-sveta.md" %}
[bloki-sveta.md](bloki-sveta.md)
{% endcontent-ref %}
{% endcolumn %}

{% column %}
{% content-ref url="podpis-predmetov.md" %}
[podpis-predmetov.md](podpis-predmetov.md)
{% endcontent-ref %}
{% endcolumn %}
{% endcolumns %}

{% columns %}
{% column %}
{% content-ref url="izmenenie-rosta.md" %}
[izmenenie-rosta.md](izmenenie-rosta.md)
{% endcontent-ref %}
{% endcolumn %}

{% column %}
{% content-ref url="vypadenie-golov.md" %}
[vypadenie-golov.md](vypadenie-golov.md)
{% endcontent-ref %}
{% endcolumn %}
{% endcolumns %}

{% columns %}
{% column %}
{% content-ref url="peremeshenie-mezhdu-mirami.md" %}
[peremeshenie-mezhdu-mirami.md](peremeshenie-mezhdu-mirami.md)
{% endcontent-ref %}
{% endcolumn %}

{% column %}
{% content-ref url="seen.md" %}
[seen.md](seen.md)
{% endcontent-ref %}
{% endcolumn %}
{% endcolumns %}

{% columns %}
{% column %}
{% content-ref url="marikhuana-i-kosyak.md" %}
[marikhuana-i-kosyak.md](marikhuana-i-kosyak.md)
{% endcontent-ref %}
{% endcolumn %}

{% column %}

{% endcolumn %}
{% endcolumns %}

{% columns %}
{% column %}

{% endcolumn %}

{% column %}

{% endcolumn %}
{% endcolumns %}
